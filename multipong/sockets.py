        ...
        for rm in rooms:
            if isPlayer:
                if len(rm.players) < MAX_ROOM_SIZE:
                    room = rm
                    room.players.add(session.sid)  # TODO: Replace with Player object's id
                    break
                elif len(rm.spectators) < MAX_ROOM_SIZE:
                    room = rm
                    room.spectators.add(session.sid)  # TODO: Replace with Player object's id
                    break
        session['room'] = room.id
        room.save()
        join_room(str(room.id))
        if bool(app.config['DEBUG_MODE']):
            print("EVENT: roomjoin: user '{}' joined room '{}'. session id: {}, session: {}".format(session.get('username', "None"), room.id, session.sid, session))
        if "username" in session and session['username'] is not None:
            emit('roomjoin', {"username": session['username'], "room": room.id}, room=str(room.id))
    else:
